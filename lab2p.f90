...
implicit none
    ...
    if (t >= 4.40 .and. t <= 23.34) then	! non si rifiuta
      ...
      ind = 1				        ! code della funzione di chi quadro. I due estremi nella condizione della dichiarazione 
						...
						! if valgono per chi quadro a sedici gradi di libertà. (6.908,28.845), alpha = 0.05, nu = 16
                                                                                                    !  (3.8157,21.920), alpha = 0.05, nu = 11
...
program prog_3
    ...
    real				:: m, q, varm, varq, cov, rho, q_quad, rin, rout, minm, maxm, minq, maxq, minrc, maxrc,     &
                                                   
                                            minv0, maxv0, dev, m_s, q_s, varm_s, varq_s, cov_s, rho_s, rc_s, v0_s, k, medg, varg

    integer				:: i, d, j, cin, cout, ind, acc, ref, ind_s

    real, parameter			:: pi = 3.14159, del = 0.1, m_par = -6.115E-3, q_par = 1.293

    character(len=40)			:: fnm, fnq, fnrc, fnv0

    real, dimension(4)			:: a_med, a_var

    real, dimension(14)		        :: s, u, v, logv, devlog, t_s, v_s, logv_s, devlog_s 

    real, dimension(:), allocatable     :: a_m, a_q, a_varm, a_varq, a_cov, a_rho, a_v0, a_rc
...
! Minimi quadrati sui dati raccolti ********************************
    ...
    open(unit = 701, file = "tempi.txt")

    read(701,*) t_s
  

    open(unit = 702, file = "tensioni.txt")
...
! Chi quadro sui dati raccolti *************************************
    ...
    call chiquad(m_s,q_s,t_s,logv_s,devlog_s,ind_s,k)

    print*, ""

    print*,"Il valore di chi quadro dato dal test sul campione vale t =", k
    ...
    case(2)		! Ipotesi rigettabile per alpha = 0.05.
      ...
      print*, "Ipotesi rigettata con significatività del 5%."
...
! SIMULAZIONI DEI SET DI MISURE ************************************
    ...
    do j = 1,d
      ...
      q_quad = (  (m-m_s)**2/varm  +  (q-q_s)**2/varq  -  2.0*rho*((m-m_s)*(q-q_s)) / sqrt(varm*varq)  )/(1-rho**2)
...
! Generazione degli istogrammi *************************************
    ...
    write(201,*) "set arrow from ", m_s,",0 to ", m_s,",100000 nohead lc rgb 'black'"

    write(201,*) "set arrow from ", medg,",0 to ", medg,",100000 nohead lc rgb 'blue'"

    write(201,*) "plot '", trim(fnm), "' w boxes lc rgb 'red', gauss(x) lc rgb 'black'"
    ...
    write(202,*) "set arrow from ", q_s,",0 to ", q_s,",100000 nohead lc rgb 'black'"

    write(202,*) "set arrow from ", medg,",0 to ", medg,",100000 nohead lc rgb 'blue'"

    write(202,*) "plot '", trim(fnq), "' w boxes lc rgb 'green', gauss(x) lc rgb 'black'"
    ...
    write(203,*) "set arrow from ", rc_s,",0 to ", rc_s,",100000 nohead lc rgb 'black'"

    write(203,*) "set arrow from ", medg,",0 to ", medg,",100000 nohead lc rgb 'blue'"

    write(203,*) "plot '", trim(fnrc), "' w boxes lc rgb 'brown', gauss(x) lc rgb 'black'"
    ...
    write(204,*) "set arrow from ", v0_s,",0 to ", v0_s,",100000 nohead lc rgb 'black'"

    write(204,*) "set arrow from ", medg,",0 to ", medg,",100000 nohead lc rgb 'blue'"

    write(204,*) "plot '", trim(fnv0), "' w boxes, gauss(x) lc rgb 'black'"
